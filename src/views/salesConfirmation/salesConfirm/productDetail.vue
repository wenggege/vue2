        ...
        <el-tab-pane v-for="(item, index) in tab" :key="index" :label="OrderNoList" :name="String(index)">
          <field-set title='基本信息' :borderBottom="false" id="BaseInfo">
            <el-descriptions :column="4" class="base_info">
              ...
              <el-descriptions-item label="PO号">{{ item.orderInfo.PONumber || '' }}
              </el-descriptions-item>
              ...
              <el-descriptions-item label="交付场地">{{ item.orderInfo.SiteName || ''
              }}</el-descriptions-item>
              <el-descriptions-item label="售达方">{{ item.orderInfo.SoldSide || '' }}</el-descriptions-item>
            </el-descriptions>
            <div v-if="item.orderInfo.salePaths !== null && item.orderInfo.salePaths.length" class="sale_title"><div class="sale_title_img"></div>销售路径</div>
            <!-- 销售路径 -->
            <div class="sale-paths" v-if="item.orderInfo.salePaths !== null && item.orderInfo.salePaths.length" style="margin-left: 0px;height: 100%;">
              ...
              <div class="sale_main">
                ...
                <div class="sale_main_two" style="height:115px">
                  <div class="sale_main_two_title"><div class="sale_main_two_title_img"></div>
              ...
              <div v-if="item.orderInfo.tradeList.length" v-for="(element, index) in item.orderInfo.tradeList"
                ...
                <div class="sale_main">
                  ...
                  <div class="sale_main_two" style="width:244px;height:115px">
                    <div class="sale_main_two_title"><div class="sale_main_two_title_img2"></div><div>{{element?.TradeSubject}}</div>
                    </div>
                    <el-descriptions :column="1" class="sale_main_two_content">
              ...
              <div class="sale_main">
                ...
                <div class="sale_main_two" style="height:115px">
                  <div class="sale_main_two_title"><div class="sale_main_two_title_img3"></div>
              ...
              <el-collapse v-model="activeNames">
                <el-collapse-item v-for="(ele, index) in item.deliveryPlanInfos" :key="index"
                  ...
                  <el-descriptions :column="4" class="sale_info">
                    ...
                    <el-descriptions-item label="交货单号">{{ ele.DeliveryNoList || '' }}
                    </el-descriptions-item>
                    <el-descriptions-item label="物流发票号">{{ ele.InvoiceNo || '' }}</el-descriptions-item>
                    <el-descriptions-item label="实现销售日期">{{ ele.AchieveSalesDate ?
                        $moment(ele.AchieveSalesDate).format('YYYY-MM-DD') : ''
                    }}</el-descriptions-item>
                    <el-descriptions-item label="SAP物料代码">{{ ele.SAPMaterialCode || '' }}</el-descriptions-item>
                    <el-descriptions-item label="物料描述">{{ ele.MaterialDescription || '' }}
                    </el-descriptions-item>
                    <el-descriptions-item label="数量">{{ ele.OrderCount || '' }}</el-descriptions-item>
                    <el-descriptions-item label="单位">{{ ele.Uint || '' }}</el-descriptions-item>
                    <el-descriptions-item label="交付项目类别">{{ ele.LineProjectType || '' }}</el-descriptions-item>
                    ...
                    <el-descriptions-item label="行项目总含税金额">{{ ele.FinalCustomerTotal || '' }}</el-descriptions-item>
                    <!-- <el-descriptions-item label="不含税金额">{{ ele.NoTaxMoney || '' }}</el-descriptions-item>
                    <el-descriptions-item label="税金">{{ ele.TaxMoney || '' }}</el-descriptions-item> -->
                    <el-descriptions-item label="贸易条款">{{ ele.Incoterms || '' }}</el-descriptions-item>
                    <el-descriptions-item label="红票单价">{{ ele.RedTicketPrice || '' }}</el-descriptions-item>
                    <el-descriptions-item label="红票总价">{{ ele.RedTicketTotal || '' }}</el-descriptions-item>
                  </el-descriptions>

                  <div class="sale_title" v-if="ele.salePaths !== null && ele.salePaths.length" ><div class="sale_title_img" style="margin-top:5px"></div>销售路径</div>
                  <!-- 销售路径 -->
                  <div class="sale-paths" v-if="ele.salePaths !== null && ele.salePaths.length" style="margin-left: 0px">
                    ...
                    <div class="sale_main">
                      ...
                      <div class="sale_main_two">
                        <div class="sale_main_two_title"><div class="sale_main_two_title_img" style="margin-top: 3px"></div>
                        <div>{{ ele.salePaths ? ele.salePaths.find(item => item.Seq ==0).SaleSubject : ''}}</div>
                        </div>
                        <el-descriptions :column="1" class="sale_main_two_content">
                          <el-descriptions-item label="币别">{{ ele.salePaths ? ele.salePaths.find(item => item.Seq ==
                              0).Currency : ''
                          }}
                          </el-descriptions-item>
                          ...
                          <el-descriptions-item label="含税单价">{{ ele.salePaths ? ele.salePaths.find(item => item.Seq
                              == 0).Price : ''
                          }}</el-descriptions-item>
                        </el-descriptions>
                      </div>
                    ...
                    <div v-if="ele.tradeList.length" v-for="(element, index) in ele.tradeList" :key="index">
                      <div class="sale_main">
                        ...
                        <div class="sale_main_two" style="width:244px">
                          <div class="sale_main_two_title"><div class="sale_main_two_title_img2" style="margin-top:4px"></div><div>{{element?.TradeSubject}}</div>
                          </div>
                          <el-descriptions :column="1" class="sale_main_two_content">
                            <el-descriptions-item label="币别">{{ element?.Currency  }}
                            </el-descriptions-item>
                    ...
                    <div class="sale_main">
                      ...
                      <div class="sale_main_two">
                        <div class="sale_main_two_title"><div class="sale_main_two_title_img3" style="margin-top:4px"></div>
      ...
      <div class="directory">
        <div style="position: fixed;">
          <div :class="[index == status ? 'directory_title_active' : 'directory_title']" v-for="(item, index) in htmlObj"
            @click="selectInfo(index,item.id)" v-if="index < 2">
            {{ item.title }}
          </div>
        ...
        <div class="directory_btn">
          ...
          <div class="directory_btn_item" v-if="Audit">
            <el-button type="primary" size="mini" @click="openAgree">同意</el-button>
          </div>
        </div>
      </div>
    </div>
      ...
      <el-form :model="rejectForm" :rules="rejectRules" ref="rejectForm" lable-width="100px">
        <el-form-item label="类型：" prop="checkList">
          <el-checkbox-group v-model="rejectForm.checkList">
            ...
            <el-checkbox label="物流" v-if="SalesType === '产品'"></el-checkbox>
          </el-checkbox-group>
        </el-form-item>
...
<script>
import { getsalelinfo, subSaleAuditDto } from '@/api/salesConfirmation'
    ...
    getsalelinfo() {
      ...
      getsalelinfo({ OrderNo: this.OrderNoList,SaleNo: this.SaleNo}).then((res) => {
        if (res.Code === 200) {
          ...
          this.SalesType = res.Saleinfo.SalesType
          ...
          if (this.tab.length) {
            this.tab.forEach(item => {
              ...
              if (item.deliveryPlanInfos !== null || item.deliveryPlanInfos?.length !== 0) {
                ...
                item.deliveryPlanInfos.forEach((ele, index) => {
                  this.activeNames.push(index)
                  if (ele.salePaths !== null && ele.salePaths.length) {
                    let list = ele.salePaths.filter((item, index) => {
                      return item.Seq != 100 && item.Seq != 0
                    })
                    ele.tradeList = list //贸易主体
      ...
      this.$refs.rejectForm.validate((valid) => {
        if (valid) {
          ...
          this.rejectForm.checkList.forEach((item) => {
            ...
            if (item === '物流') params.FileStatus = 0
          })
