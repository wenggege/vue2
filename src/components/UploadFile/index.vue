<template>
  <div>
    <div v-if="titleShow" class="title">{{ title }}</div>
    <el-alert
      v-if="tipsTitleShow"
      :title="tipsTitle"
      type="warning"
    ...
    <el-upload v-if="uploadShow" ref="upload" class="UploadFile" :class="{ hide: hideUpload }" :key="'upload_' + new Date().getTime()"
      ...
      <el-button :loading="uploadLoading" v-if="listType === 'text'" :type="buttonType" :size="buttonSize"
...
export default {
  ...
  props: {
    ...
    tipsTitle:{
      type: String,
      default: '申请开票必须上传PO或合同，可选上传工作完成证明'
    },
    // 是否显示上传
    uploadShow: {
      type: Boolean,
  ...
  methods: {
    ...
    beforeUpload(file) {
      ...
      if (file.name.length > 100) {
        this.$message.error('上传文件名不能超过100个字符')
        return false
      }
      if (this.limitSize) {
